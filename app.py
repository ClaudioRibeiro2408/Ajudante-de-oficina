...
with aba_orcamento:
    ...
    # Filtra o catálogo com base no tipo escolhido
    ...
    if st.session_state.itens_orcamento:
        for idx, item in enumerate(st.session_state.itens_orcamento):
            ...
            c_qtd.write(f"{item['whitespace' if 'quantidade' not in item else item['whitespace' if 'quantidade' not in item else 'whitespace' if 'quantidade' not in item else item['quantidade']]}")
    ...
    if st.session_state.itens_orcamento:
        ...
        with col_wa:
            if st.button("🚀 Formatar Texto de Venda (WhatsApp)", use_container_width=True):
                texto_itens = "".join([f"- {i['tipo']}: {i['descricao']} ({i['whitespace' if 'quantidade' not in i else i['whitespace' if 'quantidade' not in i else i['quantidade']]]}) -> R$ {i['valor']:.2f}\n" for i in st.session_state.itens_orcamento])
...
# ==========================================
# NOVA ABA 4: ALMOXARIFADO & SERVIÇOS (BANCO DE DADOS DE ITENS)
# ==========================================
with aba_catalogo:
    ...
    with st.form("form_catalogo", clear_on_submit=True):
        ...
        desc_cat = st.text_input("Descrição do Item / Nome do Serviço:", placeholder="Ex: Óleo 5W40 508.88 ou Mão de obra troca de Amortecedores")
