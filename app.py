...
def generar_pdf_orcamento(dados_oficina, cliente, veiculo, placa, reclamacao, itens, total, bytes_logo=None):
    ...
    if bytes_logo is not None:
        try:
            ...
            image.save(img_temp)
            # Insere a logo centralizada ou no canto (ajustado para 40mm de largura)
            pdf.image(img_temp, x=85, y=10, w=40)
            pdf.ln(25) # Abre espaço abaixo da logo
            # Remove o arquivo temporário
            if os.path.exists(img_temp):
                os.remove(img_temp)
        except Exception as e:
            pass # Se der erro no formato da imagem, segue sem quebrar o PDF
...
with aba_orcamento:
    ...
    # Campo para carregar a imagem da logo em tempo de execução
    arquivo_logo = st.file_uploader("Arraste ou selecione a imagem da logo da oficina (.png ou .jpg):", type=["png", "jpg", "jpeg"])
    
    logo_para_pdf = None
    
    if arquivo_logo is not None:
        # Se carregar o arquivo, exibe a logo centralizada com design limpo
        logo_bytes = arquivo_logo.read()
        logo_para_pdf = logo_bytes # Salva na variável para injetar no gerador de PDF
        
        col_l1, col_l2, col_l3 = st.columns([1, 2, 1])
        with col_l2:
            st.image(logo_bytes, use_container_width=True)
        st.success("Logo aplicada com sucesso no painel e preparada para o PDF!")
    else:
        # Layout reserva caso não queira subir nenhuma logo naquele orçamento
        st.markdown(f"""
        <div style="background-color: #f0f2f6; 
                    padding: 20px; 
                    border-radius: 10px; 
                    text-align: center; 
                    border: 2px dashed #b3b3cc;
                    margin-bottom: 20px;">
            <span style="color: #666687; font-family: sans-serif; font-weight: bold; font-size: 14px;">
                [ NENHUMA LOGO CARREGADA - EXIBINDO TEXTO PADRÃO COMERCIAL ]
            </span>
            <h2 style="color: #1e1e2f; margin: 10px 0 0 0; font-family: sans-serif;">{oficina_nome.upper()}</h2>
        </div>
    ...
    if st.session_state.itens_orcamento:
        ...
        with col_pdf:
            # Passa a logo dinâmica (se houver) para a função do PDF
